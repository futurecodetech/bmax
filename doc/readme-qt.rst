BMAXCOIN-qt: Qt4 GUI for BMAXCOIN
===============================
...
Alternatively, install Qt Creator and open the `BMAXCOIN-qt.pro` file.

An executable named `BMAXCOIN-qt` will be built.
...
A warning for people using the *static binary* version of BMAXCOIN on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of BMAXCOIN is linked against libdb5.3.
...
installed causes BMAXCOIN-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
