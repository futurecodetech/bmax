...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BMAXCOINTEST2\r");
            Send(hSocket, "WHO #BMAXCOINTEST2\r");
        } else {
            // randomly join #BMAXCOIN00-#BMAXCOIN05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BMAXCOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BMAXCOIN%02d\r", channel_number).c_str());
        }
